...
{	
	...
	if ( __HAL_UART_GET_FLAG(&huart3, UART_FLAG_RXNE) == true )
	{
		ucCh  = (uint16_t)(huart3.Instance->DR & (uint16_t)0x01FF);
	...
	if ( __HAL_UART_GET_FLAG(&huart3, UART_FLAG_IDLE) == true )                                         //数据帧接收完毕
	{

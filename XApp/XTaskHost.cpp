...
void WiFi_rank() 
{
	U8 wifi_strength = 0;
	if(conf_rssi>=-40){
		wifi_strength = WIFI_SIGNAL_STRENGTH_5;
	}
	else if((conf_rssi<-40) && (conf_rssi>=-50)){
		wifi_strength = WIFI_SIGNAL_STRENGTH_4;
	}
	else if((conf_rssi<-50) && (conf_rssi>=-60)){
		wifi_strength = WIFI_SIGNAL_STRENGTH_3;
	}
	else if((conf_rssi<-60) && (conf_rssi>=-70)){
		wifi_strength = WIFI_SIGNAL_STRENGTH_2;
	}
	else if((conf_rssi<-70)){
		wifi_strength = WIFI_SIGNAL_STRENGTH_1;
	}

	if(WIFI_Link() == 0){
 		  wifi_strength = WIFI_SIGNAL_STRENGTH_0;
	}

	g_platform[2] = wifi_strength << 4;
}	
*/
